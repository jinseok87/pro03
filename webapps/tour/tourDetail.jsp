...
<body>
	...
	<nav class="breadcrumb" aria-label="breadcrumbs" style="border-bottom: 2px solid #ddd; padding-bottom: 8px; min-height: 48px; line-height: 48px;">
		<ul style="float: right;">
			...
			<li>
				<a href="${path1 }/GetTourListCtrl.do">전체 장소</a>
			</li>
			<li>
				<a href="${path1 }/GetTourCateListCtrl.do?cate=${dto.cate }">
			...
			<li class="is-active">
				<a href="#" aria-current="page">${dto.place }</a>
			</li>
		</ul>
	</nav>
		...
		<div class="container">
					...
					<c:if test="${not empty list }">
						<tr>
							<c:forEach items="${list }" var="pic" varStatus="status">
								<td><img src="upload/${pic.picname }" class="lazy" onerror="this.style.display='none'" alt="배너" /> <!-- onerror="this.style.display='none'"  --></td>
							</c:forEach>
						</tr>
					</c:if>
			...
			<table class="table">
				<tbody>
					...
					<tr>
						<th>장소 번호</th>
						<td><p>${dto.tourno }</p> <input type="hidden" name="tourno" id="tourno" value="${dto.tourno }" /></td>
					</tr>
					<tr>
						<th>장소명</th>
						<td>${dto.place }</td>
			...
			<div style="clear: both; margin-bottom: 20px; padding-bottom: 20px;">
				...
				<script type="text/javascript" src="http://dapi.kakao.com/v2/maps/sdk.js?appkey=105d7fce0fbe925dfc7409d4e810d44b&​libraries=services"></script>
				<script>
			...
			<div class="buttons">
				<a href="${path1 }/GetTourListCtrl.do" class="button">전체 목록</a>
				<a href="${path1 }/GetTourCateListCtrl.do?cate=${dto.cate }" class="button ">카테고리 목록</a>
				<a href="${path1 }/GetReviewCateListCtrl.do?cate=${dto.cate }" class="button ">이용후기 목록</a>
				<c:if test='${sid.equals("admin") }'>
					<a href="${path1 }/DelTourCtrl.do?no=${dto.no }" class="button "> 삭제</a>
					<a href="${path1 }/ModifyTourCtrl.do?no=${dto.no }" class="button ">수정</a>
				</c:if>
			</div>
		</div>
	</section>
